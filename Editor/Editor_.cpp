...
{
    std::ifstream cfile("editor\\editor.astral");
    std::stringstream src;
    src << cfile.rdbuf();
    cfile.close();

    GetEngine()->LoadProjectConfig(src.str(), "editor");
}
...
void EditorLayer::OnInitialize()
{
    ...
    m_EditorView = std::make_shared<EditorView>();
...
void EditorLayer::OnBeforePresent()
{
    ...
    m_Explorer->Render(GetEngine()->GetSceneManager(), m_Properties->m_PropertiesObject);
    m_Properties->Render(GetEngine()->GetTextureManager(), m_TextureManager);
    m_EditorView->Render(GetEngine()->GetRenderer());
