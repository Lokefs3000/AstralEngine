...
#include "graphics/d11/resources/D11RenderTexture.h"
...
TextureRef D11TextureManager::GetTexture(std::string path)
{
    ...
    uint32_t findExt = path.find_last_of(".");
    uint32_t findDir = path.find_last_of("\\");

    TextureRef ref = TextureRef_(m_TextureList.size(), false, path.substr(0, findExt == std::string::npos ? 0 : findExt));
...
{
    if (ref.get() != NULL && !ref->IsRenderable() && ref->IsValid() && ref->GetRefId() < m_TextureList.size()) {
...
TextureRef D11TextureManager::GetRenderTexture(RenderTextureOptions options, std::string id)
{
    for (auto entry : m_RenderTextureReference)
    {
        if (entry.first == id) {
            return entry.second;
        }
    ...
    TextureRef ref = TextureRef_(m_TextureList.size(), true, "RENDER_TEXTURE");

    std::shared_ptr<D11RenderTexture> texture = std::make_shared<D11RenderTexture>();

    m_RenderTextureList.push_back(std::make_pair(ref, texture));
    m_RenderTextureReference.push_back(std::make_pair(id, ref));
...
std::shared_ptr<IRenderTexture> D11TextureManager::GetRenderTextureFromRef(TextureRef ref)
{
    if (ref->IsRenderable() && ref->IsValid() && ref->GetRefId() < m_RenderTextureList.size()) {
        return m_RenderTextureList[ref->GetRefId()].second;
    }
