...
void Engine::LoadProjectConfig(std::string config, std::string parent)
{
	ProjectConfig = Configurations::LoadConfig(config);
	m_ProjectFolder = parent;
}
...
void Engine::Initialize()
{
	...
	case GraphicsAPI::Direct3D11:
		...
		m_EngineTextures = std::make_shared<D11TextureManager>();
		break;
#endif
	...
	if (ProjectConfig->GetChild("ProjectConfig")->GetChild("AssetSettings")->GetValue<bool>("IsRaw")) {
		...
		m_EngineManager = std::make_shared<AssetManagerRaw>();
	}
	...
	m_EngineTextures->Initialize(m_GraphicsContext, m_EngineManager, false);

	m_Renderer->InitializeRenderer(ProjectConfig->GetChild("ProjectConfig")->GetChild("RenderSettings"), m_MainWindow, m_GraphicsContext, m_EngineTextures);
...
void Engine::Run()
{
	...
	while (!m_MainWindow->WasCloseRequested())
	{
		...
		m_Renderer->RenderMain();
		Watches.RenderWatch->End();

		for (auto layer : EngineLayers)
			layer->OnBeforePresent();
